...
cmdOptions::OptionParser_shmsOptics::OptionParser_shmsOptics() :
  displayHelp(false), automatic(false),
  rootFileName("out.root"), delay(2000),
  configFileName()
{}
...
void cmdOptions::OptionParser_shmsOptics::init(
  const int& argc, const char* const* argv
) {
  int operands = 0;
...
void cmdOptions::OptionParser_shmsOptics::printHelp() {
  std::cout << "Usage: shms_optics [OPTION]... CONFIG_F" << std::endl << std::endl;
  std::cout << "CONFIG_F : configuration file name" << std::endl;
  std::cout << "[OPTION] :" << std::endl;
  std::cout << "  -h : display this help" << std::endl;
  std::cout << "  -a : proceed automatically, do not wait for input" << std::endl;
